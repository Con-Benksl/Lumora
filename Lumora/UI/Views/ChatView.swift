...
struct ChatView: View {
    ...
    init(
        ...
        // Show cached messages immediately while Codex still syncs in the task below.
    ...
    private var chatInputPlaceholder: String {
        ...
        if canSendMessages {
            return String(localized: "Message to \(name)... (⏎ send · ⌃F/⌃B scroll · ⌃G bottom)")
        } else {
            return String(localized: "Open \(name) in tmux to enable messaging")
        }
    }
    ...
    private var interactivePromptSubtitle: String {
        ...
        case .codex:
            return String(localized: "Codex needs your input")
        case .opencode:
            return String(localized: "OpenCode needs your input")
        case .cursor:
            return String(localized: "Cursor needs your input")
        }
    }
    ...
    private func focusTerminal() {
        ...
        Task {
            ...
            if focusSucceeded {
                ...
                focusErrorMessage = String(localized: "Couldn't focus terminal. Switch to it manually (session.pid missing or terminal app not in the known list).")
            }
        }
    }
            ...
            HStack(spacing: 6) {
                ...
                if tool.isSubagentContainer {
                    if !tool.subagentTools.isEmpty {
                        let taskDesc = tool.input["description"] ?? String(localized: "Running agent...")
                    ...
                    Text(blocking ? String(localized: "Waiting: \(desc)") : desc)
                        .font(.system(size: 11))
                        .foregroundColor(textColor.opacity(0.7))
                        .lineLimit(1)
                        .truncationMode(.tail)
            ...
            if hiddenCount > 0 {
                Button {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(isExpanded
                             ? String(localized: "Hide \(hiddenCount) older tool uses")
                             : String(localized: "Show all \(tools.count) tool uses"))
                            .font(.system(size: 10))
                    }
                    .foregroundColor(secondaryTextColor)
                }
...
struct ChatInteractivePromptBar: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            .help(canFocusTerminal
                  ? String(localized: "Focus the terminal window running \(providerName)")
                  : String(localized: "Start \(providerName) inside tmux to focus the terminal from here"))
    ...
    private var providerSubtitle: String {
        switch provider {
        case .claude: return String(localized: "Claude Code needs your input")
        case .codex: return String(localized: "Codex needs your input")
        case .opencode: return String(localized: "OpenCode needs your input")
        case .cursor: return String(localized: "Cursor needs your input")
        }
    }
    ...
    private var hintSubtitle: String {
        String(localized: "Start \(providerName) in tmux to focus terminal")
    }
}
    ...
    var body: some View {
        Button(action: onTap) {
            HStack(spacing: 6) {
                ...
                Text(count == 1 ? String(localized: "1 new message") : String(localized: "\(count) new messages"))
                    .font(.system(size: 12, weight: .medium))
            }
