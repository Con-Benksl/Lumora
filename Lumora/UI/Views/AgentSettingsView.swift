...
struct AgentSettingsView: View {
    ...
    private var claudeDirPickerOptions: some View {
        VStack(spacing: 2) {
            SettingsSubPickerRow(
                label: String(localized: "Auto-detect"),
            ...
            SettingsSubPickerRow(
                label: String(localized: "Choose folder…"),
    ...
    private func hooksToggle(provider: SessionProvider, hooksOn: Bool) -> some View {
        ...
        let label: String = (provider == .opencode) ? String(localized: "Plugin") : String(localized: "Hooks")
    ...
    /// to `/tmp/lumora-debug.log` (10 MB rolling, recreated on launch).
    ...
    private var debugLogSection: some View {
        ...
        return Button {
            ...
            HStack(spacing: 10) {
                ...
                Text(debugLogOn ? String(localized: "On") : String(localized: "Off"))
                    .font(.system(size: 11))
                    .foregroundColor(secondaryTextColor)
            }
        ...
        .help("Mirror internal log output to /tmp/lumora-debug.log (10 MB, rotated). Restart the app to clear the file.")
    }
    ...
    private func openClaudeFolderPicker() {
        ...
        panel.title = String(localized: "Choose Claude Config Directory")
        panel.message = String(localized: "Select the folder Claude Code uses (typically ~/.claude or ~/.config/claude).")
        ...
        // A shortcut may have closed Lumora while the picker owned focus.
        notchWindow?.ignoresMouseEvents = MenuBarOrganizer.shared.isExpanded || MenuBarOrganizer.shared.isYieldingToSystem
            || (viewModel.status != .opened && !viewModel.isClosing)
