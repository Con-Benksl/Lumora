...
struct WriteResultContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            // Action and filename
            HStack(spacing: 4) {
                Text(result.type == .create ? String(localized: "Created") : String(localized: "Wrote"))
...
struct TaskResultContent: View {
    ...
    private func formatDuration(_ ms: Int) -> String {
        if ms >= 60000 {
            return String(localized: "\(ms / 60000)m \((ms % 60000) / 1000)s")
        } else if ms >= 1000 {
            return String(localized: "\(ms / 1000)s")
        }
...
struct KillShellResultContent: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text(result.message.isEmpty ? String(localized: "Shell \(result.shellId) terminated") : result.message)
                .font(.system(size: 11, design: .monospaced))
                .foregroundColor(.white.opacity(0.5))
        }
    }
