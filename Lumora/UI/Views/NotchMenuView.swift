...
struct NotchMenuView: View {
    ...
    /// Compile-time layout for the menu page. 15 visible rows + 5
    /// dividers (Back, divider, Screen, Sound, Agents..., Performance...,
    /// Keyboard..., divider, Appearance, Music Edge, Vibe, divider,
    /// Launch, Accessibility, Beta Features..., divider, Star, divider, Organizer, Quit).
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: true) {
            VStack(spacing: 4) {
                ...
                MenuRow(
                    icon: "keyboard",
                    label: String(localized: "Keyboard Shortcuts..."),
                    trailingIcon: "chevron.right",
                    primaryTextColor: primaryTextColor,
                ...
                MenuToggleRow(
                    icon: "music.note",
                    label: String(localized: "Music Edge Glow"),
                ...
                MenuToggleRow(
                    icon: "sparkles",
                    label: String(localized: "Vibe Glow"),
                ...
                MenuRow(
                    icon: "testtube.2",
                    label: String(localized: "Beta Features..."),
                    trailingIcon: "chevron.right",
                    primaryTextColor: primaryTextColor,
                ...
                MenuRow(
                    icon: "star",
                    label: String(localized: "Open-source project"),
                    trailingLabel: appVersion,
                    primaryTextColor: primaryTextColor,
                    isFocused: viewModel.settingsFocusedIndex == 12
                ) {
                    if let url = URL(string: "https://github.com/Con-Benksl?tab=repositories") {
            ...
            .overlay(alignment: .topLeading) {
                ScrollViewOverlayStyle(
                    isActive: viewModel.status == .opened && viewModel.contentType == .menu,
                    onMeasure: handleMeasuredContentHeight
                )
        ...
        .onAppear {
            ...
            // The scroll view's measurement will re-converge and
        ...
        .onChange(of: viewModel.contentType) { oldValue, newValue in
            ...
            if newValue == .menu {
                ...
                soundSelector.isPickerExpanded = false
                isAppearancePickerExpanded = false
                // Re-arm the content-height calibration. The local observer
                // will re-converge and rewrite menuContentHeight
    ...
    private func performFocusedAction() {
        ...
        case 13: MenuBarOrganizer.shared.showSettings()
        case 14: NSApplication.shared.terminate(nil)
        default: break
        }
    }
    ...
    /// Receive content-height samples from this menu's scroll view.
    /// The observer reads the documentView's actual frame at 10Hz while
    /// the menu is visible, so this is the most accurate height
    ...
    private func handleMeasuredContentHeight(_ height: CGFloat) {
        guard viewModel.status == .opened, viewModel.contentType == .menu else { return }
        defer {
            if lastBroadcastHeight != height { lastBroadcastHeight = height }
        }
    ...
    private var appVersion: String {
        ...
        let name = Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String ?? "Lumora"
...
struct UpdateRow: View {
    ...
    private var label: String {
        ...
        case .upToDate:
            return String(localized: "Check for Updates")
        case .found:
            return String(localized: "Download Update")
        case .downloading:
            return String(localized: "Downloading...")
        case .extracting:
            return String(localized: "Extracting...")
        case .readyToInstall:
            return String(localized: "Install & Relaunch")
        case .installing:
    ...
    var body: some View {
        ...
        Button(action: openAccessibilitySettings) {
            HStack(spacing: 10) {
                ...
                Text(currentlyEnabled ? String(localized: "Enabled") : String(localized: "Disabled"))
                    .font(.system(size: 11))
...
struct AppearanceStylePickerRow: View {
    ...
    var body: some View {
        ExpandableSettingsRow(
            ...
            label: String(localized: "Appearance Style"),
    ...
    private func sublabel(for style: NotchAppearanceStyle) -> String {
        switch style {
        case .liquidGlass:
            return String(localized: "macOS 26+ glass")
        case .adaptiveArtwork:
            return String(localized: "Dynamic music colors")
        case .pureBlack:
