...
private extension SessionListView {
    func syncLayoutMetrics() {
        ...
        // A newly inserted list has no measurements yet. Keep the last known
        // heights so reopening does not collapse to fallback sizes for one frame.
        if instanceRowHeight > 0 && abs(viewModel.instancesPageRowHeight - instanceRowHeight) > 0.5 {
            viewModel.instancesPageRowHeight = instanceRowHeight
        }

        if performanceRowHeight > 0 && abs(viewModel.instancesPagePerformanceRowHeight - performanceRowHeight) > 0.5 {
            viewModel.instancesPagePerformanceRowHeight = performanceRowHeight
        }

        if musicCardHeight > 0 && abs(viewModel.instancesPageMusicCardHeight - musicCardHeight) > 0.5 {
...
struct InstanceRow: View {
    ...
    private var phaseStatusText: String {
        ...
        case .waitingForTerminalApproval:
            return String(localized: "Approval needed in terminal")
        case .idle:
