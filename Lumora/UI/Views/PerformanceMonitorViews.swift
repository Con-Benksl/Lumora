...
struct PerformanceDetailView: View {
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: true) {
            VStack(spacing: 8) {
                MenuRow(
                    icon: "chevron.left",
                    label: section == .overview ? String(localized: "Back") : String(localized: "Performance"),
    ...
    private var cpuContent: some View {
        VStack(spacing: 8) {
            PerformanceHeroPanel(
                ...
                subtitle: String(localized: "Total processor activity"),
            ...
            PerformanceTrendPanel(
                title: String(localized: "Usage Trend"),
            ...
            PerformanceStatGrid(items: [
                PerformanceStatItem(label: String(localized: "Cores"), value: "\(Foundation.ProcessInfo.processInfo.processorCount)"),
                PerformanceStatItem(label: String(localized: "Active"), value: "\(Foundation.ProcessInfo.processInfo.activeProcessorCount)"),
    ...
    private var memoryContent: some View {
        VStack(spacing: 8) {
            PerformanceHeroPanel(
                ...
                subtitle: String(localized: "\(PerformanceFormat.percent(snapshot.memory.usage)) used"),
    ...
    private var batteryContent: some View {
        VStack(spacing: 8) {
            ...
            PerformanceTrendPanel(
                title: String(localized: "Charge Trend"),
                subtitle: historyWindowText,
                series: [
                    PerformanceChartSeries(
                        id: "battery",
            ...
            PerformanceStatGrid(items: [
                PerformanceStatItem(label: String(localized: "Source"), value: batterySourceText),
                PerformanceStatItem(label: String(localized: "Charging"), value: snapshot.battery.level == nil ? "-" : (snapshot.battery.isCharging ? String(localized: "Yes") : String(localized: "No"))),
                PerformanceStatItem(label: String(localized: "Remaining"), value: batteryTimeText),
                PerformanceStatItem(label: String(localized: "Updated"), value: PerformanceFormat.sampleAge(snapshot.sampledAt)),
            ], primaryTextColor: primaryTextColor, secondaryTextColor: secondaryTextColor)

            PerformanceInfoPanel(
                title: String(localized: "Power State"),
                rows: [
                    (String(localized: "Level"), batteryValue),
                    (String(localized: "Status"), batteryDetail),
                    (String(localized: "Adapter"), snapshot.battery.isPluggedIn ? String(localized: "Connected") : String(localized: "Disconnected")),
                ],
                primaryTextColor: primaryTextColor,
                secondaryTextColor: secondaryTextColor
            )
    ...
    private var networkContent: some View {
        VStack(spacing: 8) {
            PerformanceHeroPanel(
                ...
                subtitle: String(localized: "Current total throughput"),
                progress: nil,
                tint: TerminalColors.cyan,
                primaryTextColor: primaryTextColor,
                secondaryTextColor: secondaryTextColor
            ...
            PerformanceStatGrid(items: [
                PerformanceStatItem(label: String(localized: "Received"), value: PerformanceFormat.bytes(snapshot.network.receivedBytes)),
                PerformanceStatItem(label: String(localized: "Sent"), value: PerformanceFormat.bytes(snapshot.network.sentBytes)),
            ], primaryTextColor: primaryTextColor, secondaryTextColor: secondaryTextColor)
    ...
    private var batterySourceText: String {
        ...
        return snapshot.battery.isPluggedIn ? String(localized: "Power") : String(localized: "Battery")
    }
    ...
    private var batteryDetail: String {
        ...
        guard battery.level != nil else {
            return String(localized: "No battery reported")
        }

        if battery.isCharging {
            if let minutes = battery.timeRemainingMinutes {
                return String(localized: "Charging, full in \(PerformanceFormat.duration(minutes))")
            }
        ...
        if let minutes = battery.timeRemainingMinutes {
            return String(localized: "\(PerformanceFormat.duration(minutes)) remaining")
        }
...
private struct PerformanceOverviewCard: View {
    ...
    private var value: String {
        ...
        case .battery:
            guard let level = snapshot.battery.level else { return String(localized: "AC") }
            return PerformanceFormat.percent(level)
        case .network:
    ...
    private var detail: String {
        switch section {
        case .overview:
            return String(localized: "Updated \(PerformanceFormat.sampleAge(snapshot.sampledAt))")
        case .cpu:
            return String(localized: "Active \(Foundation.ProcessInfo.processInfo.activeProcessorCount)/\(Foundation.ProcessInfo.processInfo.processorCount) cores")
        case .memory:
            return String(localized: "\(PerformanceFormat.percent(snapshot.memory.usage)) used")
        case .battery:
            if snapshot.battery.isCharging { return String(localized: "Charging") }
            return snapshot.battery.isPluggedIn ? String(localized: "Power") : String(localized: "Battery")
        case .network:
            return String(localized: "\(snapshot.network.interfaces.count) interfaces")
        }
    }
...
private struct PerformanceMemoryBreakdownPanel: View {
    ...
    private var leftItems: [PerformanceStatItem] {
        [
            PerformanceStatItem(label: String(localized: "Physical Memory"), value: PerformanceFormat.bytes(memory.totalBytes)),
            PerformanceStatItem(label: String(localized: "Memory Used"), value: PerformanceFormat.bytes(memory.usedBytes)),
            PerformanceStatItem(label: String(localized: "Cached Files"), value: PerformanceFormat.bytes(memory.cachedFilesBytes)),
            PerformanceStatItem(label: String(localized: "Swap Used"), value: PerformanceFormat.bytes(memory.swapUsedBytes)),
        ]
    }

    private var rightItems: [PerformanceStatItem] {
        [
            PerformanceStatItem(label: String(localized: "App Memory"), value: PerformanceFormat.bytes(memory.appMemoryBytes)),
            PerformanceStatItem(label: String(localized: "Wired Memory"), value: PerformanceFormat.bytes(memory.wiredMemoryBytes)),
            PerformanceStatItem(label: String(localized: "Compressed"), value: PerformanceFormat.bytes(memory.compressedBytes)),
        ]
    }
    ...
    static func groups(from processes: [PerformanceProcessSnapshot]) -> [PerformanceProcessGroup] {
        ...
        return grouped.map { key, processes in
            ...
            return PerformanceProcessGroup(
                id: key,
                name: appPath.map(PerformanceProcessApplicationResolver.displayName(forAppPath:)) ?? (sortedProcesses.first?.name ?? String(localized: "Process")),
                appPath: appPath,
...
private enum PerformanceProcessApplicationResolver {
    ...
    static func displayName(forAppPath appPath: String) -> String {
        ...
        return fileName.isEmpty ? String(localized: "Application") : fileName
    }
...
private struct PerformanceProcessGroupView: View {
    ...
    private var groupSubtitle: String {
        ...
        return String(localized: "\(group.processes.count) processes")
    }
}
...
private enum PerformanceFormat {
    ...
    nonisolated static func duration(_ minutes: Int) -> String {
        ...
        return remainingMinutes == 0 ? String(localized: "\(hours)h") : String(localized: "\(hours)h \(remainingMinutes)m")
    }
    ...
    nonisolated static func historyWindow(_ history: [PerformanceHistorySample]) -> String {
        ...
        return String(localized: "Last \(compactDuration(seconds))")
    }
