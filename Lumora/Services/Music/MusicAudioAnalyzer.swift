...
nonisolated private final class MusicAudioAnalysisWorker: @unchecked Sendable {
    ...
    private let capture = LumoraSystemAudioCapture()
    private let queue = DispatchQueue(
        label: "com.conbenksl.lumora.music-audio-analysis",
        qos: .userInitiated
    )
    ...
    func start(
        ...
        queue.async { [self] in
            ...
            guard let processor = MusicSignalProcessor(sampleRate: capture.sampleRate) else {
                let error = NSError(
                    domain: "com.conbenksl.lumora.audio-analysis",
                    code: -1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "Unable to initialize music frequency analysis.")]
                )
                capture.stop()
                onStarted(.failure(error))
                return
