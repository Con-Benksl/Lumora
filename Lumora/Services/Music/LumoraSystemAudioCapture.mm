#import "LumoraSystemAudioCapture.h"
...
NSString *const LumoraAudioCaptureErrorDomain = @"com.conbenksl.lumora.audio-capture";
...
NSError *CaptureError(OSStatus status, NSString *operation) {
    ...
    return [NSError errorWithDomain:LumoraAudioCaptureErrorDomain
                               code:status
...
@implementation LumoraSystemAudioCapture {
...
- (NSError * _Nullable)startForBundleIdentifier:(NSString * _Nullable)bundleIdentifier {
    ...
    if (targetProcesses.count > 0) {
        ...
        // Lumora itself is excluded to prevent feedback from notification sounds.
        tapDescription = [[CATapDescription alloc]
            initMonoGlobalTapButExcludeProcesses:AudioProcessIDs(nil, false)];
    }
    tapDescription.name = @"Lumora Music Glow";
    ...
    if (!isFloat32PCM) {
        NSError *error = status == kAudioHardwareNoError
            ? [NSError errorWithDomain:LumoraAudioCaptureErrorDomain
                                  code:-1
    ...
    NSString *aggregateUID = [NSString stringWithFormat:@"com.conbenksl.lumora.music-glow.%@",
        NSUUID.UUID.UUIDString];
    NSDictionary *aggregateDescription = @{
        @kAudioAggregateDeviceNameKey: @"Lumora Music Glow Audio",
    ...
    if (!WaitForInputStreams(_state->aggregateDeviceID)) {
        NSError *error = [NSError errorWithDomain:LumoraAudioCaptureErrorDomain
