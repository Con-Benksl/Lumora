...
//  `/tmp/lumora-debug.log`. Designed for diagnosing agent-hook and
...
final class DebugLog {
    ...
    /// and from external scripts (e.g. `tail -F /tmp/lumora-debug.log`).
    static let fileURL: URL = URL(fileURLWithPath: "/tmp/lumora-debug.log")
    ...
    private let queue = DispatchQueue(label: "com.conbenksl.lumora.debuglog", qos: .utility)
    private let oslog = Logger(subsystem: "com.conbenksl.lumora", category: "DebugLog")
