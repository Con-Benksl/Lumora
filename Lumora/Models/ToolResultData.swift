...
struct BashResult: Equatable, Sendable {
    ...
    var displayOutput: String {
        ...
        return String(localized: "(No content)")
    }
}
...
struct ToolStatusDisplay {
    ...
    static func running(for toolName: String, input: [String: String]) -> ToolStatusDisplay {
        ...
        case .read:
            return ToolStatusDisplay(text: String(localized: "Reading..."), isRunning: true)
        case .edit:
            return ToolStatusDisplay(text: String(localized: "Editing..."), isRunning: true)
        case .write:
            return ToolStatusDisplay(text: String(localized: "Writing..."), isRunning: true)
        case .bash:
            ...
            return ToolStatusDisplay(text: String(localized: "Running..."), isRunning: true)
        case .grep, .glob:
            if let pattern = input["pattern"] {
                return ToolStatusDisplay(text: String(localized: "Searching: \(pattern)"), isRunning: true)
            }
            return ToolStatusDisplay(text: String(localized: "Searching..."), isRunning: true)
        case .webSearch:
            if let query = input["query"] {
                return ToolStatusDisplay(text: String(localized: "Searching: \(query)"), isRunning: true)
            }
            return ToolStatusDisplay(text: String(localized: "Searching..."), isRunning: true)
        case .webFetch:
            return ToolStatusDisplay(text: String(localized: "Fetching..."), isRunning: true)
        case .task:
            if let desc = input["description"], !desc.isEmpty {
                return ToolStatusDisplay(text: desc, isRunning: true)
            }
            return ToolStatusDisplay(text: String(localized: "Running agent..."), isRunning: true)
        case .todoWrite:
            return ToolStatusDisplay(text: String(localized: "Updating todos..."), isRunning: true)
        case .enterPlanMode:
            return ToolStatusDisplay(text: String(localized: "Entering plan mode..."), isRunning: true)
        case .exitPlanMode:
            return ToolStatusDisplay(text: String(localized: "Exiting plan mode..."), isRunning: true)
        case .askUserQuestion:
            return ToolStatusDisplay(text: String(localized: "Waiting for answer..."), isRunning: true)
        default:
            return ToolStatusDisplay(text: String(localized: "Running..."), isRunning: true)
        }
    }
    ...
    static func completed(for toolName: String, result: ToolResultData?) -> ToolStatusDisplay {
        guard let result = result else {
            return ToolStatusDisplay(text: String(localized: "Completed"), isRunning: false)
        }

        switch result {
        case .read(let r):
            let lineText = r.totalLines > r.numLines
                ? String(localized: "\(r.numLines)+ lines")
                : String(localized: "\(r.numLines) lines")
            return ToolStatusDisplay(text: String(localized: "Read \(r.filename) (\(lineText))"), isRunning: false)

        case .edit(let r):
            return ToolStatusDisplay(text: String(localized: "Edited \(r.filename)"), isRunning: false)

        case .write(let r):
            let text = r.type == .create
                ? String(localized: "Created \(r.filename)")
                : String(localized: "Wrote \(r.filename)")
            return ToolStatusDisplay(text: text, isRunning: false)

        case .bash(let r):
            if let bgId = r.backgroundTaskId {
                return ToolStatusDisplay(text: String(localized: "Running in background (\(bgId))"), isRunning: false)
            }
        ...
        case .grep(let r):
            return ToolStatusDisplay(text: String(localized: "Found \(r.numFiles) files"), isRunning: false)

        case .glob(let r):
            if r.numFiles == 0 {
                return ToolStatusDisplay(text: String(localized: "No files found"), isRunning: false)
            }
            return ToolStatusDisplay(text: String(localized: "Found \(r.numFiles) files"), isRunning: false)

        case .todoWrite:
            return ToolStatusDisplay(text: String(localized: "Updated todos"), isRunning: false)
        ...
        case .webSearch(let r):
            ...
            return ToolStatusDisplay(text: String(localized: "Searched in \(time)"), isRunning: false)

        case .askUserQuestion:
            return ToolStatusDisplay(text: String(localized: "Answered"), isRunning: false)

        case .bashOutput(let r):
            return ToolStatusDisplay(text: String(localized: "Status: \(r.status)"), isRunning: false)

        case .killShell:
            return ToolStatusDisplay(text: String(localized: "Terminated"), isRunning: false)

        case .exitPlanMode:
            return ToolStatusDisplay(text: String(localized: "Plan ready"), isRunning: false)
